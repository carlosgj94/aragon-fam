...
contract MockL2CrossDomainMessenger is IL2CrossDomainMessenger, Test {
  ...
  function relayMessage(address _target, address, bytes memory _message, uint256) external override {
    sender = msg.sender;
  ...
  function sendMessage(address _target, bytes calldata _message, uint32 _gasLimit)
    external
    override
  {
    sender = msg.sender;
