...
contract DAOProxy is Initializable, ReentrancyGuard {
  ...
  event Executed(
    Action[] actions, uint256 allowFailureMap, uint256 failureMap, bytes[] execResults
  );

  modifier onlyParentDAO() {
    require(
      msg.sender == address(bridge) && bridge.xDomainMessageSender() == parentDAO, "Not parent DAO"
    );
    _;
  }
  ...
  function execute(Action[] calldata _actions, uint256 _allowFailureMap)
    external
    onlyParentDAO
    nonReentrant
  {
    ...
    for (uint256 i = 0; i < _actions.length;) {
      ...
      (bool success, bytes memory result) =
        _actions[i].to.call{value: _actions[i].value}(_actions[i].data);
      gasAfter = gasleft();
